  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <div v-else-if="error" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <div class="text-center">
        ...
        <h3 class="mt-4 text-lg font-medium text-gray-900">Post not found</h3>
        <p class="mt-2 text-gray-500">{{ error }}</p>
        <div class="mt-6">
          <BaseButton @click="router.push('/blog')">
            Back to Blog
          </BaseButton>
        </div>
      </div>
    ...
    <!-- Blog Post Content -->
    <article v-else-if="post" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div class="lg:col-span-3">
          ...
          <header class="bg-white rounded-lg shadow-md p-8 mb-8">
            <div class="space-y-4">
              ...
              <div class="flex items-center space-x-4 text-sm text-gray-500">
                <span v-if="post.category" class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-primary-100 text-primary-800">
                  {{ post.category }}
                </span>
              ...
              <!-- Summary -->
              <p v-if="post.summary" class="text-xl text-gray-600 leading-relaxed">
                {{ post.summary }}
              </p>
          ...
          <!-- Navigation -->
          <nav v-if="previousPost || nextPost" class="bg-white rounded-lg shadow-md p-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <!-- Previous Post -->
              <div v-if="previousPost">
                <span class="text-sm font-medium text-gray-500">Previous Post</span>
                <BaseCard
                  hoverable
                  clickable
                ...
                >
                  <div class="space-y-2">
                    ...
                    <p class="text-sm text-gray-600 line-clamp-2">
                      {{ previousPost.summary || extractSummary(previousPost.content) }}
                    </p>
                  </div>
                </BaseCard>
              </div>
              ...
              <div v-if="nextPost" :class="{ 'md:text-right': !previousPost }">
                <span class="text-sm font-medium text-gray-500">Next Post</span>
                <BaseCard
                  hoverable
                  clickable
                  class="mt-2 cursor-pointer"
        ...
        <div class="lg:col-span-1 space-y-6">
          <!-- Table of Contents -->
          <BaseCard v-if="tableOfContents.length > 0" title="Table of Contents">
            <nav class="space-y-2">
              <a
          ...
          <!-- Related Posts -->
          <BaseCard v-if="relatedPosts.length > 0" title="Related Posts">
            <div class="space-y-4">
              <article
              ...
              >
                <div class="space-y-2">
                  <h4 class="font-medium text-sm text-gray-900 group-hover:text-primary-600 transition-colors line-clamp-2">
                    {{ relatedPost.title }}
                  </h4>
          ...
          <!-- Back to Blog -->
          <BaseCard>
            <BaseButton variant="outline" class="w-full" @click="router.push('/blog')">
...
function generateTableOfContents() {
  ...
  const headings = formattedContent.value.match(/<h([1-6])[^>]*id="([^"]*)"[^>]*>(.*?)<\/h[1-6]>/g)
...
onMounted(async () => {
  try {
    const identifier = route.params.slug as string || route.params.id as string
    
    if (!identifier) {
      error.value = 'No post identifier provided'
      return
    }
