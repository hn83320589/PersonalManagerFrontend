...
// 獲取元件內的文字內容
export const getTextContent = (wrapper: VueWrapper<any>, selector: string): string => {
  const element = wrapper.find(selector)
  return element.exists() ? element.text() : ''
}

// 檢查元件是否包含特定 class
export const hasClass = (wrapper: VueWrapper<any>, selector: string, className: string): boolean => {
  const element = wrapper.find(selector)
...
// 點擊按鈕的輔助函數
export const clickButton = async (wrapper: VueWrapper<any>, selector: string) => {
